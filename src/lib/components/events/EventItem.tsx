...
import {Badge, Box, ButtonBase, Paper, SxProps, Theme, Typography, useTheme,} from "@mui/material"
            ...
            {showdate && (
                <Typography style={{fontSize: 11}} noWrap>
                    {`${format(event.start, "H:mm", {
                        locale: locale,
                    })} - ${format(event.end, "H:mm", {locale: locale})}`}
                </Typography>
            )}
        </Item>
    )
            ...
            <Multiday className='Multiday'>
                <Typography sx={{fontSize: 11}} noWrap>
                    {hasPrev ? (
                        ...
                        showdate && format(event.start, "H:mm", {locale: locale})
                    )}
                </Typography>
                <Typography
                    variant="subtitle2"
                ...
                <Typography sx={{fontSize: 11}} noWrap>
                    {hasNext ? (
                        ...
                        showdate && format(event.end, "H:mm", {locale: locale})
                    )}
                </Typography>
            </Multiday>
        )
